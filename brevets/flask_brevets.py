...
def get_brev():
    ...
    brevet = lists[-1]
    return brevet["brev_dist"], brevet["start_time"], brevet["items"]
...
def insert_brev(brev_dist, start_time, items):
    ...
    """
    _id = requests.post(f"{API_URL}/brevets", json={"brev_dist": brev_dist, "start_time": start_time, "items": items}).json()
    return str(_id) 
